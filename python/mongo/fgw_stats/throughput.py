...
class Throughput(object) :
  ...
  def dotiming(self, duration) :
    ...
    time1 = int(time.time())
    for result in mongo.FileGateway.audit_messages.aggregate({'$group': {'_id': '$status', 'total': { '$sum': 1 }}})['result'] :
  ...
  def dorange(self, start, end) :
    ...
    mongo = self.get_mongo()
    for result in mongo.FileGateway.audit_messages.aggregate([{'$match' : {'audit_time' : {'$gte': start }}},
      {'$match' : {'audit_time' : {'$lte': end }}},{'$group':{'_id' : '$status', 'total': { '$sum': 1}} }])['result'] :
      if result['_id'] == u'SUCCESS' :
        total = result['total']
